...
function Notification() {
   ...
   return (
      <div className=" w-full h-full mx-auto p-6">
            ...
            {tabs.map((tab) => (
               <button
                  ...
                  className={`py-2 px-4 text-sm font-medium rounded-full transition-all ${
                     activeTab === tab.id ? "bg-[#2D2E2D] text-white" : "bg-[#161716] text-gray-400 hover:text-gray-300"
                  }`}
               >
                  {tab.label}
               </button>
         ...
         <div className="text-white flex flex-col gap-3 py-5 px-8">{renderComponent()}</div>
      </div>
   );
}
