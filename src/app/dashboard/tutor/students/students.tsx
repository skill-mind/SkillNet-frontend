...
function Student() {
  ...
  return (
    <div className="overflow-y-auto flex-grow ml-[20px] w-[calc(100vw-372px)] bg-[#101110] text-white">
      <main className="flex-1 flex-grow w-full">
          ...
          <table className="w-full">
            <thead className="h-[48px]">
              <tr className="text-left text-sm bg-[#161716] p-10 box-border border-zinc-800">
                ...
                <th className="font-normal text-zinc-400 py-[17px] px-[9px]">Courses</th>
                <th className="font-normal text-zinc-400 py-[17px] px-[9px]">Email Address</th>
                <th className="font-normal text-zinc-400 py-[17px] px-[9px]">Progress</th>
              </tr>
            </thead>
            <tbody>
              {students.map((student) => (
                <tr key={student.id} className="border-b h-[68px] border-zinc-800">
                  <td className="py-[17px] px-[9px] whitespace-nowrap">{student.name}</td>
                  <td className="py-[17px] px-[9px] whitespace-nowrap">{student.course}</td>
                  <td className="py-[17px] px-[9px] whitespace-nowrap">{student.email}</td>
                  <td className="py-[17px] px-[9px] whitespace-nowrap">{student.progress}%</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className='overflow-y-auto'>
          ...
          <div className="space-y-[14px]">
            {reviews.map((review) => (
              <div key={review.id} className="flex border-b border-[#2D2E2D] py-3 flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[14px] text-white font-medium mt-1">{review.review}</p>
                </div>
                <div className="flex gap-2 align-center text-right sm:self-center">
                  <div className="text-sm text-zinc-400">Rating:</div>
                  <div className="font-medium">{review.rating}/{review.rating}</div>
                </div>
              </div>
            ))}
          </div>
